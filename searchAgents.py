...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        cornersLeft = tuple(corner for corner in state[1] if corner != state[0]) 
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                cornersLeft = tuple(c for c in state[1] if c != state[0])
                successorState = ((nextx,nexty), cornersLeft)
...
def mazePath(point1, point2, gameState):
    ...
    assert not walls[x2][y2], 'point2 is a wall: ' + str(point2)
    prob = PositionSearchProblem(gameState, start=point1, goal=point2, warn=False, visualize=False)
    return search.aStarSearch(prob)
